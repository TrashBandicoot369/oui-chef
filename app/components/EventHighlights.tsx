...
export default function EventHighlights({ events }: { events: Event[] }) {
  const validEvents = events.filter((e) => e?.image && e?.alt)
  if (validEvents.length < events.length && process.env.NODE_ENV !== 'production') {
    console.warn(`EventHighlights: skipped ${events.length - validEvents.length} invalid event(s)`)
  }
        ...
        <div className="flex-1 mx-12 flex items-center justify-center gap-16">
          <div className="flex-1 max-w-lg">
            <h2 className="text-3xl font-bold uppercase mb-4 text-accent2">
              {validEvents[index].alt ?? 'Event image'}
            </h2>
            <p className="text-base leading-relaxed text-accent2">
              {validEvents[index].description}
            </p>
          </div>
  ...
  >
              ...
              <image
                ...
                aria-label={validEvents[index].alt ?? 'Event image'}
              />
            </svg>
          </div>
        </div>
