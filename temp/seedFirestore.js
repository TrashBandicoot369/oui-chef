...
async function wipe(col) {
    ...
    const snap = await firebase_admin_1.db.collection(col).get();
...
Alex`,
    ...
    for (const event of events) {
        await firebase_admin_1.db.collection('eventHighlights').add({
            ...event,
            visible: true,
            createdAt: firestore_1.FieldValue.serverTimestamp(),
            updatedAt: firestore_1.FieldValue.serverTimestamp()
    ...
    for (const item of menuItems) {
        await firebase_admin_1.db.collection('menuItems').add({
            ...item,
            visible: true,
            createdAt: firestore_1.FieldValue.serverTimestamp(),
            updatedAt: firestore_1.FieldValue.serverTimestamp()
    ...
    for (const testimonial of testimonials) {
        await firebase_admin_1.db.collection('testimonials').add({
            ...testimonial,
            visible: true,
            createdAt: firestore_1.FieldValue.serverTimestamp(),
            updatedAt: firestore_1.FieldValue.serverTimestamp()
    ...
    for (const image of galleryImages) {
        await firebase_admin_1.db.collection('gallery').add({
            ...image,
            visible: true,
            createdAt: firestore_1.FieldValue.serverTimestamp(),
            updatedAt: firestore_1.FieldValue.serverTimestamp()
    ...
    await wipe('theme');
    await firebase_admin_1.db.doc('theme/colors').set({
        primary1: '#1c1b20',
        primary2: '#383234',
        primary3: '#3f393c',
        accent1: '#e3973b',
    ...
    await firebase_admin_1.db.doc('theme/fonts').set({
        display: 'Anton',
        sans: 'Bitter',
        button: 'Oswald',
        createdAt: firestore_1.FieldValue.serverTimestamp(),
